...
def add_db(result, cfg):
    ...
    with psql.cursor() as cur:
        ...
        arg = cur.mogrify("(NOW(),%d)", size)
        cur.execute(f"INSERT INTO {tbl} VALUES" + arg.decode('utf-8'))

        cur.execute(f"SELECT ts,size FROM {tbl} ORDER BY id DESC LIMIT 40")
        result["db"]["data"] = [ {'ts': t, 'size': s} for t, s in cur.fetchall() ]
...
def main():
    ...
    if os.path.isfile(sys.argv[2]):
        ...
        if "db" in prev and "prev-date" in prev["db"]:
            ...
            log_files = {"prev-date": prev_date, "data": prev["db"]["data"]}
    ...
    if "db" in cfg and run_db:
        try:
            ...
            print("Unable to monitor the DB", e)
